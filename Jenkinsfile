...
pipeline {
    ...
    stages {
        ...
        stage('Run Docker Container') {
            steps {
                bat '''
                docker stop %CONTAINER_NAME% 2> nul || echo Container not running, continuing...
                docker rm %CONTAINER_NAME% 2> nul || echo No container to remove, continuing...
                docker run -d --name %CONTAINER_NAME% -p 5555:8080 %IMAGE_NAME%:%IMAGE_TAG%
                '''
            }
        }
